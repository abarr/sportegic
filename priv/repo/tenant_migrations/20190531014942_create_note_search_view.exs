    ...
    execute(
      """
        CREATE MATERIALIZED VIEW #{prefix()}.notes_search_view AS
          SELECT n.id AS id, n.subject AS subject, (
            setweight(to_tsvector(n.subject), 'B') ||
            setweight(to_tsvector(coalesce(string_agg(t.name, ' '), ' ')), 'A')
          ) AS search
      ...
      execute("CREATE INDEX notes_search_title_trgm_index ON #{prefix()}.notes_search_view USING gin (subject gin_trgm_ops)")
