defmodule SportegicWeb.PerformanceController do
  ...
  def new(conn, _params, person, org, permissions) do
    with :ok <- Bodyguard.permit(Profiles, "create:performance_permissions", "", permissions) do  
  ...
  def create(conn, %{"performance" => performance_params, "profile_id" => profile_id, "person_id" => person_id}, _person, org, permissions) do
    with :ok <- Bodyguard.permit(Profiles, "create:performance_permissions", "", permissions) do 
  ...
  def show(conn, %{"id" => id}, _person, org, permissions) do
    with :ok <- Bodyguard.permit(Profiles, "view:performance_permissions", "", permissions) do 
      performance = Profiles.get_performance!(id, org)
      render(conn, "show.html", performance: performance)
    end
  end

  def edit(conn, %{"id" => id}, _person, org, permissions) do
    with :ok <- Bodyguard.permit(Profiles, "edit:performance_permissions", "", permissions) do 
      performance = Profiles.get_performance!(id, org)
      changeset = Profiles.change_performance(performance)
  ...
  def update(conn, %{"id" => id, "performance" => performance_params}, person, org, permissions) do
    with :ok <- Bodyguard.permit(Profiles, "edit:performance_permissions", "", permissions) do 
      performance = Profiles.get_performance!(id, org)
      ...
      case Profiles.update_performance(performance, performance_params, org) do
        {:ok, performance} ->
          ...
          |> redirect(to: Routes.person_profile_performance_path(conn, :show, person, performance))
  ...
  def delete(conn, %{"id" => id}, person, org, permissions) do
    with :ok <- Bodyguard.permit(Profiles, "delete:performance_permissions", "", permissions) do 
      performance = Profiles.get_performance!(id, org)
      {:ok, _performance} = Profiles.delete_performance(performance, org)
