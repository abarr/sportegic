defmodule SportegicWeb.DocumentController do
  ...
  def index(conn, _params, person, org, permissions) do
    with :ok <- Bodyguard.permit(People, "view:document_permissions", "", permissions) do
  ...
  def new(conn, _params, person, org, permissions) do
    with :ok <- Bodyguard.permit(People, "create:document_permissions", "", permissions) do
  ...
  def create(conn, %{"document" => document_params}, person, org, permissions) do
    with :ok <- Bodyguard.permit(People, "create:document_permissions", "", permissions) do
      document_params =
        document_params
  ...
  def edit(conn, %{"id" => id}, person, org, permissions) do
    with :ok <- Bodyguard.permit(People, "edit:document_permissions", "", permissions) do
  ...
  def update(conn, %{"id" => id, "document" => document_params}, person, org, permissions) do
     with :ok <- Bodyguard.permit(People, "edit:document_permissions", "", permissions) do
      document = People.get_document!(person, id, org)
  ...
  def delete(conn, %{"id" => id}, person, org, permissions) do
    with :ok <- Bodyguard.permit(People, "delete:document_permissions", "", permissions) do
      document = People.get_document!(person, id, org)
